...
服务器版：[B 版欢迎页](https://app.chainvalley.top/zhijing-b/?start=welcome) · [重新走问卷](https://app.chainvalley.top/zhijing-b/?start=questionnaire)。
服务器独立常驻，关闭本机或 Codex 不影响它；本地预览命令仍是前台运行，两者不要混淆。
...
直接打开 [服务器上的知行](https://app.chainvalley.top/zhijing-b/zhixing/) 会回到最近的场景；本地体验则需要本地网页和 API 均在运行。记录存储在独立 SQLite 中，不只是页面缓存；同一访客会话可持续使用，沿用恢复码机制转移访问。服务器版不会自动导入本机或 A 版的记录，不承诺服务器永不离线。
...
| 网页 / API | 本地 `3100` / `4430`；服务器仅监听 `127.0.0.1:4359` / `4360`          |
| 数据       | 本地 `data-plan-b/`；服务器 B 版独立持久化目录，不带原版账户或记录       |
| 会话       | `zhijing_plan_b_session`；线上 Cookie 路径 `/zhijing-b`，浏览器存储使用 B 专属前缀 |
| 配置       | 本目录 `.env.plan-b`；拒绝导入其他项目的配置或数据目录                    |
| 部署       | 服务器独立 `/zhijing-b/`，systemd 常驻、自启、异常重启；A 版服务与数据不变 |

本地保持 `/zhijing/`，正式构建使用 `NEXT_PUBLIC_BASE_PATH=/zhijing-b`；案例、猫猫入口和返回链接都沿用本次构建的路径。部署与维护见 [部署说明](deploy/README.md)。
...
源码与重建案例所需媒体随仓库提供；不包含密钥、用户数据库、恢复码、私人对话和本地依赖。GitHub 是源码入口，运行中的 B 版在上方服务器链接；推送代码不会自动部署。当前测试验证功能与数据边界，**不证明个性化学习或行动建议已提升真实生活效果**。
